...
#### Для GitHub Pages:
1. Скопируйте `config.github.example.js` в `config.github.js`
2. Измените пароль в файле `config.github.js`
3. Файл `config.github.js` добавлен в `.gitignore` и не попадет в публичный репозиторий

#### ⚠️ ВАЖНО для безопасности:
- **НЕ коммитьте** файлы с паролями в публичный репозиторий
- **Используйте приватный репозиторий** для продакшена
- **Регулярно меняйте пароли** для админ-панели
