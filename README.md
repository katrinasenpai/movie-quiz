...
2. Измените пароль в файле `config.js`
3. Файл `config.js` добавлен в `.gitignore` и не попадет в публичный репозиторий

#### Для GitHub Pages:
- **Пароль зашифрован** в коде с использованием XOR шифрования
- **Ключ шифрования** скрыт в коде
- **Пароль по умолчанию**: `lib2025` (зашифрован)

#### 🔒 Безопасность:
- **Пароль не виден** в открытом виде в коде
- **Используется шифрование** для защиты пароля
- **Репозиторий публичный** для работы GitHub Pages
- **Регулярно меняйте пароль** в коде при необходимости

### Доступ к админ-панели
- **Секретная комбинация**: нажмите клавиши `a-d-m-i-n` по порядку
- **Консольная команда**: `showAdminPanel()` в консоли браузера
- **Смена пароля**: `changePassword()` в консоли браузера
